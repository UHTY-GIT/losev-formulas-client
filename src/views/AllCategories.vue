<template>
  ...
  >
    <div class="block-podcasts">
      <div class="title-category" v-if="category.name === selectedCategory || selectedCategory === 'all'">
        <p>
...
export default {
  ...
  methods: {
    ...
    applyFilters() {
      ...
      // });
      this.podcasts.forEach(podcast => {
        podcast.isVisible = (this.selectedCategory === 'all' || podcast.categories[0].name === this.selectedCategory) &&
            (this.selectedFilterPrice === 'Всі подкасти' || podcast.categories[0].category_type === (this.selectedFilterPrice === 'Безкоштовні' ? 'free' : 'paid'));
      });
    },
  ...
  mounted() {
    ...
    Promise.all([
      ...
      apiService.allCategories()
        ...
        .then(([favoritePodcastsResponse, responseAllPodcast, responseCategories]) => {

<!--scr/components/app/MyPodcast.vue-->
<template>
  ...
  <div class="all-podcast-block">
    <div v-if="podcasts.length === 0" class="no-podcasts-message">
      <p>
        Тут відображатимуться куплені подкасти.<br>
        Придбайте свій перший подкаст <router-link to="/">вже зараз</router-link>.
      </p>
    </div>
    <div class="block_content_podcast" v-else>
      <div
...
export default {
  ...
  methods: {
    ...
    //анімація при переході між cторінками, шоб подкаст відображався шо він грає
    startAnimation() {
  ...
  mounted() {
    ...
    Promise.all([
      ...
      apiService.getUserPodcasts(token)
    ])
        .then(([favoritePodcastsResponse, userPodcastsResponse]) => {
          console.log('favoritePodcastsResponse:', favoritePodcastsResponse);
          console.log('userPodcastsResponse:', userPodcastsResponse.data);
          ...
          this.podcasts = userPodcastsResponse?.data?.map(podcast => ({
            ...podcast,
            isVisible: true,
  ...
  async created() {
    ...
    // Отримуємо данні моїх подкастів
    // const responseMyPodcast = await apiService.getUserPodcasts(token)
    //
    // if(responseMyPodcast && responseMyPodcast.data && responseMyPodcast.data.data) {
    //   this.podcasts = responseMyPodcast.data.data.map(podcast => {
    //     return {
    //       ...podcast,
    //       isVisible: true,
